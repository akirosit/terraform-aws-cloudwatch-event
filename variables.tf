...
variable "create_target" {
  description = "Whether a target should be created (defaults to false)."
  type        = bool
  nullable    = false
  default     = false
}
...
variable "target_event_bus_name" {
  description = "(Optional) The event bus to associate with the target. If you omit this, the default event bus is used."
  type        = string
  default     = null
}
